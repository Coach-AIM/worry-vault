...
describe('ExportPdfButton Component', () => {
  ...
  it('triggers PDF export and formats task and journal entry content', async () => {
    ...
    const mockTasks = [
      { id: 1, title: 'Read therapist notes', emotionalIntensity: 'low', estimatedTime: '15m', completed: 1, createdAt: today }
    ];
    const mockEntries = [
      {
        id: 10,
    ...
    await waitFor(() => {
      ...
      expect(mockText).toHaveBeenCalledWith(expect.stringContaining('Read therapist notes'), 25, expect.any(Number));
      // Should write situation text
      expect(mockSplitTextToSize).toHaveBeenCalledWith(expect.stringContaining('Work stress'), 170);
      // Should write reframed thought text
      expect(mockSplitTextToSize).toHaveBeenCalledWith(expect.stringContaining('I can do it step-by-step'), 170);
      expect(mockSave).toHaveBeenCalledWith('momentum-weekly-summary.pdf');
    });
  });
