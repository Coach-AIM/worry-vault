...
export default function ExportPdfButton() {
  ...
  async function handleExport() {
    ...
    try {
      ...
      const recentTasks = tasks.filter(
        (t: any) => new Date(t.createdAt) >= oneWeekAgo && t.completed,
      );
      const recentEntries = entries.filter(
        (e: any) => new Date(e.createdAt) >= oneWeekAgo,
      );
      ...
      if (recentTasks.length === 0) {
        ...
        recentTasks.forEach((t: any) => {
          doc.text(
            `• ${t.title} (${t.emotionalIntensity} Intensity, ${t.estimatedTime || "N/A"})`,
            25,
            y,
          );
          y += 8;
      ...
      if (recentEntries.length === 0) {
        ...
        recentEntries.forEach((e: any) => {
          ...
          doc.setFontSize(11);
          doc.text(
            `Date: ${date} - [${isPositive ? "🏆 Victory Reflection" : "💭 CBT Thought Record"}]`,
            20,
            y,
          );
          y += 7;
          ...
          const sitLines = doc.splitTextToSize(
            `${sitHeader}${e.situation}`,
            170,
          );
          ...
          if (!isPositive && e.automaticThought) {
            const thoughtLines = doc.splitTextToSize(
              `Thought: ${e.automaticThought}`,
              170,
            );
            ...
            try {
              ...
              if (Array.isArray(emotions) && emotions.length > 0) {
                const emotionNames = emotions
                  .map((em: any) => `${em.name} (${em.weight}%)`)
                  .join(", ");
                const emotionLines = doc.splitTextToSize(
            ...
            try {
              ...
              if (Array.isArray(distortions) && distortions.length > 0) {
                const distortionNames = distortions
                  .map((id: string) => DISTORTION_NAMES[id] || id)
                  .join(", ");
                const distortionLines = doc.splitTextToSize(
                  `Thinking Traps: ${distortionNames}`,
                  170,
                );
          ...
          const reframeHeader = isPositive
            ? "Anchor/Strength: "
            : "Reframed Thought: ";
          const reframeLines = doc.splitTextToSize(
            `${reframeHeader}${e.reframedThought}`,
            170,
          );
  ...
  return (
    <button
      ...
      disabled={exporting}
      className="secondary"
      style={{ width: "100%", fontSize: "1.05rem", padding: "0.875rem" }}
    >
