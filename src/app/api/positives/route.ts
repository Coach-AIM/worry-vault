...
export async function GET() {
  try {
    const glimmers = await db.select().from(positiveThoughts).orderBy(desc(positiveThoughts.createdAt));
    return NextResponse.json({ glimmers });
  } catch (error) {
    console.error("DB Positives Fetch Error", error);
    return NextResponse.json({ error: "Failed to fetch positive thoughts" }, { status: 500 });
  }
}
...
export async function POST(req: Request) {
  try {
    ...
    return NextResponse.json({ error: "Failed to save positive thought" }, { status: 500 });
  }
}
