...
export async function PATCH(
  ...
  try {
    ...
    if (isNaN(entryId)) {
      return new NextResponse(JSON.stringify({ error: "Invalid Entry ID" }), {
        status: 400,
        headers: { "Content-Type": "application/json" },
      });
    }
    ...
    const existing = await db
      ...
      .where(
        and(
          ...
          eq(journalEntries.userId, currentUserId),
        ),
      );
    ...
    const updated = await db
      ...
      .set({
        ...
        predictionEvaluation:
          predictionEvaluation !== undefined ? predictionEvaluation : null,
      })
      .where(
        and(
          eq(journalEntries.id, entryId),
