...
export async function GET() {
  try {
    ...
    const expiredList = await db.select()
      .from(decisions)
      .where(
    ...
    return NextResponse.json({ error: err.message || "Failed to fetch expired decisions." }, { status: 500 });
  }
}
...
export async function POST(req: Request) {
  try {
    ...
    if (!decisionId || !chosenOptionId || !actualFeeling) {
      return NextResponse.json({ error: "Missing decisionId, chosenOptionId, or actualFeeling." }, { status: 400 });
    }
    ...
    return NextResponse.json({ error: err.message || "Failed to submit decision follow up." }, { status: 500 });
  }
}
